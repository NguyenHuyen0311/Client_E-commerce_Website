  ...
  return (
    <div className="blog-item group shadow-md mb-5 rounded-sm">
      ...
      <div className="blog-content px-3 pb-3 mt-4">
        <h2 className="text-[16px] font-[500] text-black line-clamp-2 text-ellipsis">
            <Link to={`/blog-details/${props?.item?._id}`} className="link">
              {props?.item?.title}
            </Link>
        </h2>
        ...
        <Link to={`/blog-details/${props?.item?._id}`} className="py-1 flex items-center gap-1 link font-[400] text-[15px]">
            Đọc thêm
